...
class Like {
  ...
  toggleHeartClick(e) {
    ...
    if (this.likeBox.getAttribute('data-exists') == 'yes') this.removeLike(curBox);else this.addLike(curBox);
  }
  async addLike(element) {
    try {
      const {
        data
      } = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(`${universityData.rootUrl}/wp-json/university/v1/manage-like`, {
  ...
  async removeLike(element) {
    try {
      await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](`${universityData.rootUrl}/wp-json/university/v1/manage-like/?like=${element.getAttribute('data-like')}`);
      const likeCountBox = element.querySelector('.like-count');
...
class Search {
  ...
  events() {
    this.openButton.forEach(btn => btn.addEventListener('click', this.openOverlay.bind(this)));
  ...
  async getResults() {
    const data = await (await fetch(`${universityData.rootUrl}/wp-json/university/v1/search?term=${this.searchField.value}`)).json();
    const html = `
      <div class="row">
        <div class="one-third">
  ...
  keyPressDispatcher(e) {
    if ((document.activeElement.tagName === 'INPUT' || document.activeElement.tagName === 'TEXTAREA') && document.activeElement !== this.searchField) return;
...
class MyNotes {
  ...
  async deleteNote(e) {
    ...
    try {
      const {
        data
      } = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](`${universityData.rootUrl}/wp-json/wp/v2/note/${thisNote.getAttribute('data-id')}`);
      ...
      if (data.userNoteCount) document.querySelector('.note-limit-message').classList.remove('active');
    } catch (err) {
      console.log(err);
    }
  }
  async createNote(e) {
    ...
    try {
      const {
        data
      } = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(`${universityData.rootUrl}/wp-json/wp/v2/note`, newPost);
  ...
  async updateNote(e) {
    ...
    try {
      await axios__WEBPACK_IMPORTED_MODULE_0___default().post(`${universityData.rootUrl}/wp-json/wp/v2/note/${thisNote.getAttribute('data-id')}`, updatedPost);
